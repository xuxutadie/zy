...
def public_user_label(phone: str, anonymous: bool) -> str:
    if anonymous:
        return "匿名用户"
...
def point_balance(conn: DbConnection, user_id: int) -> int:
    row = conn.execute("SELECT COALESCE(SUM(points), 0) AS balance FROM point_logs WHERE user_id = ?", (user_id,)).fetchone()
    return int(row[0] or 0) if row else 0


def review_help_reply(conn: DbConnection, reply_id: int, admin_user_id: int, status: str) -> dict:
    ...
    conn.execute(
        ...
        WHERE id = ?
        """,
        (normalized_status, admin_user_id, reviewed_at, reviewed_at, reply_id),
    )

    if normalized_status == "approved" and reply["status"] != "approved":
        existing = conn.execute(
            "SELECT id FROM point_logs WHERE source_type = ? AND source_id = ? LIMIT 1",
            ("help_reply", reply_id),
        ).fetchone()
        if not existing:
            points_awarded = 10
            conn.execute(
                """
                INSERT INTO point_logs (user_id, points, reason, source_type, source_id, created_at)
                VALUES (?, ?, ?, ?, ?, ?)
                """,
                (reply["user_id"], points_awarded, "互帮互助回复审核通过", "help_reply", reply_id, reviewed_at),
            )

    return {"replyId": reply_id, "status": normalized_status, "pointsAwarded": points_awarded}


def help_reply_payload(row: sqlite3.Row, include_admin: bool = False) -> dict:
    item = {
        "id": row["id"],
        "postId": row["post_id"],
        "content": row["content"],
        "status": row["status"],
        "author": public_user_label(row["phone"], bool(row["anonymous"])),
        "createdAt": row["created_at"],
    ...
    if include_admin:
        item.update(
            {
                "phone": row["phone"],
                "maskedPhone": mask_phone(row["phone"]),
...
def help_post_payload(row: sqlite3.Row, replies: list[dict]) -> dict:
    return {
        "id": row["id"],
        "title": row["title"],
        "content": row["content"],
...
def handle_list_help_posts(handler: BaseHTTPRequestHandler) -> None:
    user = require_user(handler)
    if not user:
        return
    query = parse_qs(urlparse(handler.path).query)
    ...
    with db() as conn:
        post_rows = conn.execute(
            """
            SELECT help_posts.id, help_posts.user_id, help_posts.title, help_posts.content,
              help_posts.anonymous, help_posts.status, help_posts.created_at, users.phone
            FROM help_posts
            JOIN users ON users.id = help_posts.user_id
            WHERE help_posts.status = 'active'
            ORDER BY help_posts.id DESC
            LIMIT ?
            """,
            (limit,),
        ).fetchall()
        ...
        for post in post_rows:
            reply_rows = conn.execute(
                """
                SELECT help_replies.id, help_replies.post_id, help_replies.user_id, help_replies.content,
                  help_replies.anonymous, help_replies.status, help_replies.reviewed_at,
                  help_replies.created_at, users.phone
                FROM help_replies
                JOIN users ON users.id = help_replies.user_id
                WHERE help_replies.post_id = ? AND help_replies.status = 'approved'
                ORDER BY help_replies.id ASC
                """,
                (post["id"],),
            ).fetchall()
            items.append(help_post_payload(post, [help_reply_payload(reply) for reply in reply_rows]))
        balance = point_balance(conn, user["id"])
        redemptions = conn.execute(
            """
            SELECT id, institution_name, points, status, created_at
    ...
    json_response(
        ...
        200,
        {
            ...
            "rules": {
                "replyApprovedPoints": 10,
                "pointValueRmb": 1,
                "institutionPointLimit": 200,
                "reviewRequired": True,
            },
            "redemptions": [
                {
...
def handle_create_help_post(handler: BaseHTTPRequestHandler) -> None:
    user = require_user(handler)
    if not user:
        return
    payload = read_json(handler)
    ...
    anonymous = 1 if payload.get("anonymous") else 0
    if len(title) < 4 or len(title) > 80:
        json_response(handler, 400, {"ok": False, "message": "标题需为 4-80 个字"})
        return
    if len(content) < 10 or len(content) > 1000:
        json_response(handler, 400, {"ok": False, "message": "求助内容需为 10-1000 个字"})
        return
    ...
    with db() as conn:
        post_id = insert_and_get_id(
            conn,
            """
            INSERT INTO help_posts (user_id, title, content, anonymous, status, created_at, client_ip)
            VALUES (?, ?, ?, ?, ?, ?, ?)
            """,
            (user["id"], title, content, anonymous, "active", now_ts(), handler.client_address[0]),
        )
        audit_log(conn, user, "create_help_post", {"postId": post_id}, handler)
        conn.commit()

    json_response(handler, 200, {"ok": True, "message": "求助已发布", "id": post_id})


def handle_create_help_reply(handler: BaseHTTPRequestHandler) -> None:
    user = require_user(handler)
    if not user:
        return
    payload = read_json(handler)
    post_id = int_or_none(payload.get("postId"))
    content = str(payload.get("content") or "").strip()
    anonymous = 1 if payload.get("anonymous") else 0
    if not post_id:
        json_response(handler, 400, {"ok": False, "message": "缺少帖子 ID"})
        return
    if len(content) < 2 or len(content) > 1000:
        json_response(handler, 400, {"ok": False, "message": "回复内容需为 2-1000 个字"})
        return

    with db() as conn:
        post = conn.execute("SELECT id FROM help_posts WHERE id = ? AND status = 'active'", (post_id,)).fetchone()
        if not post:
            json_response(handler, 404, {"ok": False, "message": "求助帖不存在或已关闭"})
            return
        reply_id = insert_and_get_id(
            conn,
            """
            INSERT INTO help_replies (post_id, user_id, content, anonymous, status, created_at, client_ip)
            VALUES (?, ?, ?, ?, ?, ?, ?)
            """,
            (post_id, user["id"], content, anonymous, "pending", now_ts(), handler.client_address[0]),
        )
        audit_log(conn, user, "create_help_reply", {"replyId": reply_id, "postId": post_id}, handler)
        conn.commit()

    json_response(handler, 200, {"ok": True, "message": "回复已提交，待管理员审核后展示；审核通过加 10 积分。", "id": reply_id})


def handle_create_coupon_redemption(handler: BaseHTTPRequestHandler) -> None:
    user = require_user(handler)
    if not user:
        return
    payload = read_json(handler)
    institution_name = str(payload.get("institutionName") or "").strip()
    points = int_or_none(payload.get("points")) or 0
    if len(institution_name) < 2 or len(institution_name) > 80:
        json_response(handler, 400, {"ok": False, "message": "请填写要兑换的培训机构名称"})
        return
    if points < 1 or points > 200:
        json_response(handler, 400, {"ok": False, "message": "单个机构每次最多使用 200 积分"})
        return

    with db() as conn:
        balance = point_balance(conn, user["id"])
        if balance < points:
            json_response(handler, 400, {"ok": False, "message": f"当前积分不足，可用积分 {balance}"})
            return
        redemption_id = insert_and_get_id(
            conn,
            """
            INSERT INTO coupon_redemptions (user_id, institution_name, points, status, created_at)
            VALUES (?, ?, ?, ?, ?)
            """,
            (user["id"], institution_name, points, "pending", now_ts()),
        )
        conn.execute(
            """
            INSERT INTO point_logs (user_id, points, reason, source_type, source_id, created_at)
            VALUES (?, ?, ?, ?, ?, ?)
            """,
            (user["id"], -points, "兑换培训班补习折扣券", "coupon_redemption", redemption_id, now_ts()),
        )
        audit_log(conn, user, "create_coupon_redemption", {"redemptionId": redemption_id, "points": points}, handler)
        conn.commit()

    json_response(handler, 200, {"ok": True, "message": "兑换申请已提交，管理员后续核销。", "id": redemption_id})


def handle_list_admin_help_replies(handler: BaseHTTPRequestHandler) -> None:
    ...
    if status not in allowed_status:
        status = "pending"
    limit = int_or_none((query.get("limit") or ["100"])[0]) or 100
    limit = max(1, min(limit, 200))
    where = "" if status == "all" else "WHERE help_replies.status = ?"
    ...
    with db() as conn:
        rows = conn.execute(
            ...
            SELECT help_replies.id, help_replies.post_id, help_replies.user_id, help_replies.content,
              help_replies.anonymous, help_replies.status, help_replies.reviewed_at,
              help_replies.created_at, users.phone,
              help_posts.title AS post_title, help_posts.content AS post_content
            FROM help_replies
            JOIN users ON users.id = help_replies.user_id
            JOIN help_posts ON help_posts.id = help_replies.post_id
            {where}
            ORDER BY help_replies.id DESC
            LIMIT ?
            """,
            params,
        ).fetchall()
        pending_count = conn.execute("SELECT COUNT(*) FROM help_replies WHERE status = 'pending'").fetchone()[0]
        audit_log(conn, user, "list_help_replies", {"status": status, "count": len(rows)}, handler)
        conn.commit()

    json_response(
        handler,
        200,
        {"ok": True, "items": [help_reply_payload(row, include_admin=True) for row in rows], "pendingCount": pending_count},
    )


def handle_review_help_reply(handler: BaseHTTPRequestHandler) -> None:
    user = require_admin(handler)
    if not user:
        return
    payload = read_json(handler)
    reply_id = int_or_none(payload.get("id"))
    status = str(payload.get("status") or "").strip()
    if not reply_id:
        json_response(handler, 400, {"ok": False, "message": "缺少回复 ID"})
        return
    try:
        with db() as conn:
            result = review_help_reply(conn, reply_id, user["id"], status)
            audit_log(conn, user, "review_help_reply", result, handler)
            conn.commit()
    except ValueError as error:
        json_response(handler, 400, {"ok": False, "message": str(error)})
        return
    except LookupError as error:
        json_response(handler, 404, {"ok": False, "message": str(error)})
        return

    message = "回复已审核通过，用户积分 +10。" if result["pointsAwarded"] else "回复审核状态已更新。"
    json_response(handler, 200, {"ok": True, "message": message, **result})
...
class AppHandler(BaseHTTPRequestHandler):
    def do_GET(self) -> None:
        ...
        if parsed.path == "/api/auth/me":
            handle_me(self)
            return
        if parsed.path == "/api/help/posts":
            handle_list_help_posts(self)
            return
        ...
        if parsed.path == "/api/admin/calculator-submissions/export":
            ...
            return
        if parsed.path == "/api/admin/help-replies":
            handle_list_admin_help_replies(self)
            return
    ...
    def do_POST(self) -> None:
        ...
        routes = {
            ...
            "/api/help/posts": handle_create_help_post,
            "/api/help/replies": handle_create_help_reply,
            "/api/help/coupon-redemptions": handle_create_coupon_redemption,
            ...
            "/api/admin/help-reply/review": handle_review_help_reply,
        }
